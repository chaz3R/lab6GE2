...
namespace graphic_editor_2.Models.Shapes {
    public class Shape6_CompositeFigure: IShape 
    {
        ...
        { 
            PName, PCommands, PColor, PThickness, PFillColor 
        };
        ...
        public Shape? Build(Mapper map) 
        {
            ...
            if (map.GetProp(PCommands) is not SafeGeometry @commands || !@commands.Valid)
            {
                return null;
            }
        ...
        public bool Load(Mapper map, Shape shape) 
        {
            ...
            if (map.GetProp(PCommands) is not SafeGeometry @commands)
            {
                return false;
            }
        ...
        public Dictionary<string, object?>? Export(Shape shape) 
        {
            ...
            if (@path.Name == null || !@path.Name.StartsWith("sn|"))
            {
                return null;
            }
        ...
        public Shape? Import(Dictionary<string, object?> data) 
        {
            ...
            if (!data.ContainsKey("path") || data["path"] is not string @path)
            {
                return null;
            }
