...
namespace graphic_editor_2.Models {
    public class Mapper 
    {
        ...
        public string shapeColor = "Black";
        public string shapeFillColor = "Pink";
        ...
        public bool ValidInput() 
        {
            foreach (PropsN num in cur_shaper.Props)
                if (GetProp(num) is ISafe @prop && !@prop.Valid)
                {
                    return false;
                }
            return true;
        ...
        public Shape? Create(bool preview) 
        {
            ...
            if (name2shape.TryGetValue(shapeName, out var value))
            {
                Remove(value);
            }
        ...
        {
            var Name = item.Name;
            if (!shape_dict.ContainsKey(Name))
            {
                return;
            }

            var shape = shape_dict[Name];
            if (shape == null || shape.Parent is not Canvas @c)
            {
                return;
            }
        ...
        public void Export(bool is_xml) 
        {
            ...
            foreach (var item in shape_dict) 
            {
                ...
                if (R)
                {
                    Log.Write("Потеряна одна из фигур при экспортировании");
                }
            }
            if (is_xml) 
            {
                ...
                if (xml == null) 
                { 
                    Log.Write("Не получилось экспортировать в Export.xml"); 
                    return; 
                }
            ...
            else 
            {
                ...
                if (json == null) 
                { 
                    Log.Write("Не получилось экспортировать в Export.json"); 
                    return; 
                }
        ...
        public Shape[]? Import(bool is_xml) 
        {
            ...
            foreach (object? item in @list) 
            {
                ...
                { 
                    Log.Write("Одна из фигур при импорте - не словарь"); 
                    continue; 
                }

                if (!@dict.ContainsKey("type") || @dict["type"] is not string @type) 
                { 
                    Log.Write("Нет поля type, либо это поле не строка"); 
                    continue; 
                }
                if (!@dict.ContainsKey("name") || @dict["name"] is not string @shapeName) 
                { 
                    Log.Write("Нет поля name, либо это поле не строка"); 
                    continue; 
                }
                if (!TShapers.ContainsKey(@type)) 
                { 
                    Log.Write("Фигуратор " + @type + " не обнаружен"); 
                    continue; 
                }
                ...
                if (newy == null) 
                { 
                    Log.Write("Фигура не собрана " + Utils.Obj2json(@dict)); 
                    continue; 
                }
        ...
        public ShapeListBoxItem? ShapeTap(string name) 
        {
            ...
            {
                name = Utils.Base64Decode(name.Split('|')[1]);
            }
            else
            {
                return null;
