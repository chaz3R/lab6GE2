...
namespace graphic_editor_2.Models.Shapes {
    public class Shape1_Line: IShape {
        ...
        public Shape? Build(Mapper map) 
        {
            ...
            if (map.GetProp(PEndDot) is not SafePoint @end || !@end.Valid)
            {
                return null;
            }
        ...
        public bool Load(Mapper map, Shape shape) 
        {
            ...
            if (map.GetProp(PEndDot) is not SafePoint @end)
            {
                return false;
            }
        ...
        public Shape? Import(Dictionary<string, object?> data) 
        {
            ...
            if (!data.ContainsKey("start") || data["start"] is not Point @start)
            {
                return null;
            }
            if (!data.ContainsKey("end") || data["end"] is not Point @end)
            {
                return null;
            }
