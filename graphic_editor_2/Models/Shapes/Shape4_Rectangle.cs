...
namespace graphic_editor_2.Models.Shapes {
    public class Shape4_Rectangle: IShape 
    {
        ...
        { 
            PName, PStartDot, PWidth, PHeight, PColor, PThickness, PFillColor 
        };
        ...
        {
            if (map.GetProp(PName) is not string @name)
            {
                return null;
            }

            if (map.GetProp(PStartDot) is not SafePoint @start || !@start.Valid)
            {
                return null;
            }

            if (map.GetProp(PWidth) is not SafeNum @width || !@width.Valid)
            {
                return null;
            }

            if (map.GetProp(PHeight) is not SafeNum @height || !@height.Valid)
            {
                return null;
            }

            if (map.GetProp(PColor) is not string @color)
            {
                return null;
            }

            if (map.GetProp(PFillColor) is not string @fillColor)
            {
                return null;
            }

            if (map.GetProp(PThickness) is not int @thickness)
            {
                return null;
            }
        ...
        public bool Load(Mapper map, Shape shape) 
        {
            ...
            if (map.GetProp(PStartDot) is not SafePoint @start)
            {
                return false;
            }
            if (map.GetProp(PWidth) is not SafeNum @width)
            {
                return false;
            }
            if (map.GetProp(PHeight) is not SafeNum @height)
            {
                return false;
            }
        ...
        {
            if (!data.ContainsKey("name") || data["name"] is not string @name)
            {
                return null;
            }

            if (!data.ContainsKey("margin") || data["margin"] is not Thickness @margin)
            {
                return null;
            }
            if (!data.ContainsKey("width") || data["width"] is not short @width)
            {
                return null;
            }
            if (!data.ContainsKey("height") || data["height"] is not short @height)
            {
                return null;
            }

            if (!data.ContainsKey("stroke") || data["stroke"] is not SolidColorBrush @color)
            {
                return null;
            }
            if (!data.ContainsKey("fill") || data["fill"] is not SolidColorBrush @fillColor)
            {
                return null;
            }
            if (!data.ContainsKey("thickness") || data["thickness"] is not short @thickness)
            {
                return null;
            }
