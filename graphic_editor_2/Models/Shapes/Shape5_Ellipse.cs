...
namespace graphic_editor_2.Models.Shapes {
    public class Shape5_Ellipse: IShape {
        ...
        public Shape? Build(Mapper map) 
        {
            ...
            if (map.GetProp(PCenterDot) is not SafePoint @center || !@center.Valid)
            {
                return null;
            }

            if (map.GetProp(PVertDiagonal) is not SafeNum @width || !@width.Valid)
            {
                return null;
            }

            if (map.GetProp(PHorizDiagonal) is not SafeNum @height || !@height.Valid)
            {
                return null;
            }
        ...
        public bool Load(Mapper map, Shape shape) 
        {
            ...
            if (map.GetProp(PCenterDot) is not SafePoint @start)
            {
                return false;
            }
            if (map.GetProp(PVertDiagonal) is not SafeNum @width)
            {
                return false;
            }
            if (map.GetProp(PHorizDiagonal) is not SafeNum @height)
            {
                return false;
            }
